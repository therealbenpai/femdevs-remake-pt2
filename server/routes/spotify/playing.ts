...
export default defineEventHandler(async (event) => {
    ...
    const MainSpotifyRes = await SpotifyAPI.getMyCurrentPlaybackState({ market: 'US' }).catch(() => { });
    const notPlaying: SpotifyListening = {
        isPlaying: false,
        playing: {
            track: {
    ...
    const getArtist = async (artist: Partial<SpotifyApi.ArtistObjectFull>): Promise<ArtistInfo> =>
    ...
    if (!MainSpotifyRes) return notPlaying;
    const body = MainSpotifyRes.body as SpotifyApi.CurrentPlaybackResponse;
    if (!Object.hasOwn(body, 'item')) return notPlaying;
    ...
    artists.forEach(async artist => full.playing.artists.push(await getArtist(artist)))
    album.artists.forEach(async artist => full.playing.album.artists.push(await getArtist(artist)))
    return full;
});