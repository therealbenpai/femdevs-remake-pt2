...
class PermissionSet {
	permKey: string;
	bit: number;
	parentBits: Array<number>;
	bitStr: string;
	constructor(key: string, bit: number, ...parentBits: Array<number>) {
		this.permKey = key;
		this.bit = bit;
...
class UserPermissionSet {
	...
	permBitStr: string;
	enabled: boolean;
	constructor(basePermission: PermissionSet) {
...
class User {
	perms: Map<string, UserPermissionSet>;
	constructor() {
		this.perms = new Map();
		Object.values(UserPermissions.permissions)
			.forEach(v => Object.values(v)
				.forEach(perm => (perm instanceof PermissionSet)
					? this.perms.set(perm.permKey, new UserPermissionSet(perm))
					// @ts-expect-error
					: Object.values(perm).forEach(val => this.perms.set(val.permKey, new UserPermissionSet(val))),
				),
			);
	}
	givePermission = (perm: string) => this.perms.get(perm)!.enable();
	removePermission = (perm: string) => this.perms.get(perm)!.disable();
	togglePermission = (perm: string) => this.perms.get(perm)!.toggle();
	hasPermission = (perm: string, allowMgr: boolean = true) => [perm]
		.concat((allowMgr)
			? ['Owner', 'Administrator', 'Developer'].map(v => `Global::Role.${v}`)
			: [],
		)
		.some(v => this.perms.get(v)!.isEnabled());
	hasPermissions = (amgr: boolean, ...perms: string[]) => perms.some(v => this.hasPermission(v, amgr));
	...
	static fromFullPermissionString(permStr: string) {
		const user = new User();
		permStr.split(',').forEach(v => user.givePermission(v));
		return user;
	}
	static fromFullPermissionBitString(permStr: string) {
		const user = new User();
